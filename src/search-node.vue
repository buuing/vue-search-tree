...
export default {
  ...
  render () {
    const { data } = this
    return <ul style={{ margin: 0, padding: '0 0 0 15px' }}>
      <li style={{
        margin: 0,
        padding: 0,
        listStyle: 'none',
        ...
        <svg t="1585220115926" class="tree-icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2222" width="10" height="10" style={{
        ...
        <input v-model={data.checked} type="checkbox" style="cursor: pointer; transform: translateY(1px)" />
        <span style="cursor: pointer" onClick={e => console.log(data)}>{data.name}</span>
      </li>
      {
        data?.children?.length && data.expand ? data.children.map(item => {
          return <search-node key={item.id} data={item}></search-node>
        }) : null
      }
    </ul>
  }
