  ...
  render () {
    ...
    return <ul class="tree-ul">
      <li class="tree-li">
        ...
        { this.checkbox && <input v-model={data.checked} type="checkbox" class="tree-checkbox point" /> }
        <span class="tree-name point" onClick={e => console.log(data)}>{data.name}</span>
      </li>
      { !!data?.children?.length && data.expand && data.children.map(item => <search-node key={item.id} data={item} checkbox={this.checkbox}></search-node>) }
    </ul>
  }
}
</script>
